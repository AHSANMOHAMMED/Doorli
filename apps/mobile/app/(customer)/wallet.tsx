...
export default function WalletScreen() {
  ...
  async function topUp() { try { const res = await apiClient.post('/wallet/topup', { amount: Number(amount), method: 'stripe' }, { headers: { 'Idempotency-Key': `${Date.now()}-mobile-topup` } }); setMessage(res.data.data?.clientSecret ? 'Payment created. Complete Stripe checkout to credit your wallet.' : 'Payment is awaiting confirmation.'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Top-up failed.'); } }
